...
const QuestionScreen = () => {
  ...
  const { questions, currentQuestionIndex, userAnswers, timer } = useSelector(
    (state) => state.quiz
  );
  ...
  const allBlanksFilled =
    currentUserAnswers.filter(Boolean).length === blankCount;
  ...
  const handleOptionClick = (option) => {
    ...
    const blankIndex =
      emptyBlankIndex !== -1 ? emptyBlankIndex : currentUserAnswers.length;
    ...
    <div className="min-h-screen bg-gray-100 p-4 flex flex-col">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 max-w-3xl w-full mx-auto mt-8">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="text-xl font-bold text-blue-600">Time: {timer}s</div>
        </div>
